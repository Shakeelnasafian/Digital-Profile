...
class GenerateVCardAction
{
    public function __invoke(Profile $profile): array
    {
        ...
        if ($profile->job_title) {
            $lines[] = 'TITLE:'.$profile->job_title;
        }
        if ($profile->email) {
            $lines[] = 'EMAIL;TYPE=INTERNET:'.$profile->email;
        }
        if ($profile->phone) {
        ...
        if ($profile->linkedin) {
            $lines[] = 'URL;TYPE=linkedin:'.$profile->linkedin;
        }
        if ($profile->github) {
            $lines[] = 'URL;TYPE=github:'.$profile->github;
        }
        if ($profile->location) {
            $lines[] = 'ADR;TYPE=WORK:;;'.$profile->location.';;;;';
        }
        if ($profile->short_bio) {
            $lines[] = 'NOTE:'.str_replace(["\r", "\n"], ' ', $profile->short_bio);
        }
